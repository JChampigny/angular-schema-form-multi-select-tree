        ...
        function ($rootScope, $scope, $http) {
            ...
            function isItemInModel(item) {
                ...
                for (var i in $scope.selectedItems) {
                    if ($scope.selectedItems[i].hasOwnProperty("value") && item.value === $scope.selectedItems[i].value) {
            ...
            $scope.populateTitleMap = function (form, search) {
                ...
                if (!form.options) {
                    ...
                    var finalPostOptions = $scope.getOptions(form.options, search);
                    return $http.post(finalPostOptions.httpPost.url, finalPostOptions.httpPost.parameter).then(
                        function (response) {
                            $scope.finalizeTitleMap(form, response.data, finalPostOptions);
                            console.log('httpPost items', form.titleMap);
                            $scope.busy = false;
                        },
                        function (response, status) {
                            alert("Loading select items failed (URL: '" + String(finalPostOptions.httpPost.url) +
                                "' Parameter: " + String(finalPostOptions.httpPost.parameter) + "\nError: " + status);
                        });
                } else if (form.options.httpGet) {
                    var finalGetOptions = $scope.getOptions(form.options, search);
                    return $http.get(finalGetOptions.httpGet.url, finalGetOptions.httpGet.parameter).then(
                        function (response) {
                            $scope.finalizeTitleMap(form, response.data, finalGetOptions);
                            console.log('httpGet items', form.titleMap);
                            $scope.busy = false;
                        },
                        function (response, status) {
                            alert("Loading select items failed (URL: '" + String(finalGetOptions.httpGet.url) +
                                "\nError: " + status);
                        });
...
return angularSchemaFormMultiSelectTree;
}));
